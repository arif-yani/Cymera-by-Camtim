...
class ManualCamera: CameraFront {
    ...
    var flashModeAuto: AVCaptureDevice.FlashMode = .auto
    ...
    var shutterSpeedValue: CMTime! {
        didSet {
            if shutterMode == .manual {
                self.setExposureModeCustom(exposureDuration: shutterSpeedValue as! CMTime, iso: isoValue) { () -> (Void) in
                }
            }
        }
    }
...
//    func takePhoto(orientation: AVCaptureVideoOrientation, flas) {
